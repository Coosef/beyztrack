...
# Uptime Kuma Temiz Kurulum Script'i
# Sadece orijinal Uptime Kuma - hiçbir değişiklik yok
# Kullanım: curl -sSL https://raw.githubusercontent.com/Coosef/beyztrack/main/install-uptime-kuma-clean.sh | bash
...
print_logo() {
    ...
    echo -e "${NC}"
    echo -e "${GREEN}🚀 Uptime Kuma Temiz Kurulumu${NC}"
    echo ""
}
...
install_uptime_kuma() {
    info "📥 Uptime Kuma temiz kurulumu yapılıyor..."
    ...
    # Master branch'e geç
    info "📌 Uptime Kuma master branch'ine geçiliyor..."
    ...
    success "✅ Uptime Kuma temiz kurulumu tamamlandı"
}
...
main() {
    ...
    echo -e "${GREEN}"
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║             Uptime Kuma Temiz Kurulumu Tamamlandı!          ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo ""
    echo -e "${BLUE}🌐 Uptime Kuma Web Arayüz: http://$(hostname -I | awk '{print $1}')${NC}"
    echo -e "${BLUE}🌐 Uptime Kuma Web Arayüz: http://localhost${NC}"
    echo -e "${BLUE}🔧 Backend: http://localhost:3001${NC}"
    echo ""
    echo -e "${YELLOW}📝 İlk kurulum adımları:${NC}"
    echo -e "${CYAN}   1. Web arayüzüne gidin${NC}"
    echo -e "${CYAN}   2. Database seçin (SQLite önerilir)${NC}"
    echo -e "${CYAN}   3. Admin kullanıcısı oluşturun${NC}"
    echo -e "${CYAN}   4. Setup'ı tamamlayın${NC}"
    echo ""
    echo -e "${YELLOW}⚠️  Setup tamamlandıktan sonra branding yapabilirsiniz:${NC}"
    echo -e "${BLUE}   curl -sSL https://raw.githubusercontent.com/Coosef/beyztrack/main/brand-latest.sh | bash${NC}"
    echo ""
}
