    ...
    async send(notification, msg, monitorJSON = null, heartbeatJSON = null) {
        try {
            if (heartbeatJSON == null) {
                const title = "BeyzTrack Alert";
                const description = "Test notification";
                const url = "BeyzTrack Test Button";
                return this.postNotification(notification, title, msg, monitorJSON);
            }

            if (heartbeatJSON.status === UP) {
                const title = "BeyzTrack Monitor ✅ Up";
            ...
            if (heartbeatJSON.status === DOWN) {
                const title = "BeyzTrack Monitor 🔴 Down";
    ...
    async postNotification(notification, title, body, monitorInfo, eventAction = "trigger") {
        ...
        const options = {
            ...
            data: {
                ...
                dedup_key: "BeyzTrack/" + monitorInfo.id,
            }
        };

        const baseURL = await setting("primaryBaseURL");
        if (baseURL && monitorInfo) {
            options.client = "BeyzTrack";
